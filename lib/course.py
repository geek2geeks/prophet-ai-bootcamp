...
DAYS = [
    {
        ...
        "exercicios": [
            {"id": "ex0.1", "titulo": "Abrir o Terminal e Navegar sem Medo", "pontos": 10,
             "descricao": "⏱ ~15 min | Abrir o terminal (CMD/PowerShell no Windows, Terminal no Mac). Executar 5 comandos: (1) ver a pasta atual -- escrever 'cd' no Windows ou 'pwd' no Mac e carregar Enter, (2) listar ficheiros -- escrever 'dir' no Windows ou 'ls' no Mac, (3) criar uma pasta -- 'mkdir bootcamp', (4) entrar nela -- 'cd bootcamp', (5) voltar atras -- 'cd ..'. Copiar o output de cada comando para um ficheiro comandos_dia0.txt."},
            {"id": "ex0.2", "titulo": "Obter API Key e Configurar DeepSeek", "pontos": 10,
             "descricao": "⏱ ~20 min | OPCAO A (recomendada): Ir a pagina Exercicios nesta app, selecionar Dia 0, e clicar em 'Chaves do Bootcamp' para copiar a key partilhada. OPCAO B (conta propria): Ir a platform.deepseek.com, criar conta, adicionar credito (~$2), clicar 'API Keys' no menu lateral, criar nova key e copia-la IMEDIATAMENTE (so aparece uma vez!). Em ambos os casos: NUNCA colar a key no codigo -- configura-se como variavel de ambiente (ver instrucoes no Modulo 2)."},
            {"id": "ex0.3", "titulo": "Instalar OpenCode e Primeiro Teste", "pontos": 10,
             "descricao": "⏱ ~20 min | Instalar OpenCode CLI (no terminal: 'npm install -g @opencode/cli' ou seguir o guia oficial). Copiar as chaves da pagina Exercicios > Dia 0 > 'Chaves do Bootcamp'. Configurar no terminal -- Windows: 'set DEEPSEEK_API_KEY=a-tua-key' / Mac: 'export DEEPSEEK_API_KEY=a-tua-key'. Abrir o OpenCode (escrever 'opencode' no terminal). Testar com: 'Ola, sou atuario e estou a comecar o bootcamp. Confirma que estas operacional.' Documentar o resultado em setup_log.md."},
            {"id": "ex0.4", "titulo": "Analisar Dados Reais com OpenCode", "pontos": 10,
             "descricao": "⏱ ~25 min | Primeiro, descarrega os ficheiros de dados do bootcamp (o instrutor fornece o link ou estao no repositorio do curso na pasta data/day0/). Abre o OpenCode na pasta onde guardaste os ficheiros. Pedir ao OpenCode: (1) 'Abre o ficheiro carteira_vida_sample.csv e diz-me quantas apolices tem esta carteira e qual o capital total segurado em euros', (2) 'Qual e a idade media dos segurados e quantos sao fumadores?', (3) 'Resume esta carteira como se fosse para apresentar ao board de uma seguradora portuguesa'. Guardar as 3 respostas num ficheiro analise_carteira.md (para criar o ficheiro, basta pedir ao OpenCode: 'guarda estas respostas num ficheiro chamado analise_carteira.md')."},
            {"id": "ex0.5", "titulo": "Prompt Preciso vs Prompt Explorador", "pontos": 10,
             "descricao": "⏱ ~20 min | Usar o ficheiro tabua_mortalidade_CSO2017.csv (na mesma pasta dos dados do Dia 0). Testar DOIS estilos de prompt diferentes sobre os mesmos dados: PROMPT PRECISO: 'Abre o ficheiro tabua_mortalidade_CSO2017.csv. Qual e o valor exato de qx para homens e mulheres aos 65 anos? Mostra os numeros e calcula a diferenca relativa entre sexos.' PROMPT EXPLORADOR: 'Abre o ficheiro tabua_mortalidade_CSO2017.csv. Olhando para a mortalidade aos 65 anos, que historias contam estes numeros? Que implicacoes tem para um produto de seguro vida? Sugere 3 ideias de produto baseadas nestes dados.' Comparar as respostas: quando pedir numeros exactos e quando pedir interpretacao? Guardar ambas as respostas."},
            {"id": "ex0.6", "titulo": "Mapa da Stack AI do Fundador", "pontos": 10,
             "descricao": "⏱ ~15 min | Pedir ao OpenCode: 'Cria um diagrama em texto que mostre como funciona a stack do bootcamp: Terminal → OpenCode CLI → API DeepSeek → Modelo LLM → Resposta. Inclui onde entram os ficheiros locais (CSVs, PDFs) e onde sai o resultado final.' Guardar como mapa_stack.md. O objetivo e perceber quem faz o que antes de comecar a construir."},
        ],
        "desafio": {"id": "des0", "titulo": "Kit do Fundador AI", "pontos": 25,
                    "descricao": "⏱ ~30 min | Entregar um founder-kit com: (1) setup_log.md documentando toda a instalacao passo a passo, (2) 3 prompts reutilizaveis que testaste e funcionam para trabalho atuarial (ex: resumir tabuas, analisar carteiras, explicar regulacao), (3) mapa_stack.md com explicacao de cada componente, (4) um checklist pessoal de 5 regras de operacao segura com LLMs. Para submeter: cola um link para os teus ficheiros (Google Drive, pasta partilhada ou repositorio) OU descreve o que fizeste no campo de texto abaixo. Nao precisas de saber usar GitHub neste dia."},
        "conteudo": {
            "modulo_1": {
            ...
            "modulo_3": {
                ...
                "topicos": [
                    ...
                    {"titulo": "5 Regras de Seguranca para Atuarios",
                     "conteudo": "(1) NUNCA colar dados reais de clientes num LLM publico -- usar sempre dados sinteticos ou anonimizados para testes e desenvolvimento. (2) Guardar API keys FORA do codigo -- em variaveis de ambiente (que ja aprendeste no Modulo 2) ou em ficheiros .env (um ficheiro de texto simples com chaves, que fica no teu computador mas nunca vai para o repositorio graças ao .gitignore -- um ficheiro que diz ao sistema de controlo de versoes quais ficheiros ignorar). (3) Verificar SEMPRE os calculos do LLM -- ele e excelente a estruturar e explicar, mas comete erros numericos. Confiar, mas verificar. (4) Documentar o que o LLM fez -- para auditoria e compliance, registar que prompts usaste e que outputs obtiveste. (5) Separar ambientes -- o que testas localmente nao e o que vai para producao. A credibilidade do teu produto para seguros comeca aqui."},
                ]
            }
        }
    },
    {
        ...
        "exercicios": [
            {"id": "ex1.1", "titulo": "Glossario Pessoal do Fundador", "pontos": 10,
             "descricao": "Depois de ler o Modulo 1, usa o OpenCode para pesquisar e escrever definicoes curtas (1-2 frases, nas tuas palavras) para cada um destes termos: (1) SaaS, (2) MVP, (3) Wedge, (4) Workflow, (5) UX, (6) Feature, (7) Deploy, (8) Incumbent, (9) Posicionamento, (10) Stack. Para cada termo, acrescenta um exemplo concreto do mundo dos seguros. Formato: tabela com colunas [Termo | Definicao | Exemplo em Seguros]. Guardar como glossario_fundador.md. Este glossario vai ser util para o resto do bootcamp."},
            {"id": "ex1.2", "titulo": "Horas vs Produto -- Porque Construir Software", "pontos": 10,
             "descricao": "Usar o OpenCode para pesquisar e resumir: qual a diferenca entre um atuario que vende horas de consultoria e um atuario que vende um produto de software por subscricao (SaaS -- software que o cliente acede pela internet e paga mensalmente)? Pedir ao LLM para comparar em 3 dimensoes: (a) Margem -- quanto sobra de cada euro faturado, (b) Escala -- se podes crescer sem trabalhar proporcionalmente mais horas, (c) Dependencia de tempo -- se paras de trabalhar, o rendimento para? Entregar um documento curto (1 pagina max) com a tabela comparativa e 3 conclusoes pessoais. Formato: tabela com colunas [Dimensao | Vender Horas | Vender Produto] + 3 bullets de conclusao."},
        ...
        "conteudo": {
            "modulo_1": {
                ...
                "topicos": [
                    ...
                    {"titulo": "Workflow, UX, Deploy e Stack", "conteudo": "Workflow e uma sequencia de passos que alguem segue para completar uma tarefa (ex: 'abrir Excel, copiar tabua, correr macro, verificar resultados' e um workflow). UX (User Experience) e a facilidade e clareza com que alguem usa o software. Deploy e o ato de colocar o software na internet para outros o poderem usar. Stack e o conjunto de ferramentas e tecnologias que o produto usa (ex: 'Python + Streamlit + Supabase' e uma stack)."},
                ]
            },
            "modulo_2": {
                "titulo": "Porque Criar um Produto em vez de Vender Horas",
                "topicos": [
                    {"titulo": "Nao e sobre Aprender a Programar", "conteudo": "Neste bootcamp nao vais tornar-te programador. Vais aprender a usar o OpenCode e as ferramentas do Dia 0 para dizer ao computador o que construir, verificar se fez bem, e melhorar o resultado. E como ser o arquiteto de uma casa sem ter de assentar os tijolos."},
