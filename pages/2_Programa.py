...
st.header("Programa do Curso")
st.caption("10 dias / 2 semanas / 80 horas — AI & Data Science para Atuarios de Vida")
...
for day in days_filtered:
    with st.expander(f"Dia {day['dia']} — {day['titulo']}", expanded=False):
        ...
        for ex in day["exercicios"]:
            st.markdown(f"- **{ex['id'].upper()}** — {ex['titulo']} ({ex['pontos']} pts)")
            st.caption(f"  {ex['descricao']}")

        d = day["desafio"]
        st.markdown(f"##### Desafio do Dia ({d['pontos']} pts)")
